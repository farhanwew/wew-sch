...
interface PaperDetailProps {
  paper: Paper;
  onBack: () => void;
  onBuildGraph: (paper: Paper, useDeepGraph?: boolean) => void;
  isBuilding?: boolean;
}
          ...
          <div className="p-8 bg-gradient-to-br from-slate-50 to-white">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full max-w-lg">
                {/* Simple Graph */}
                <button
                  onClick={() => onBuildGraph(paper, false)}
                  disabled={isBuilding}
                  className="flex-1 px-6 py-4 bg-[#004e7c] text-white rounded-xl font-bold text-[14px] hover:bg-[#003d61] transition-all shadow-lg shadow-teal-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex flex-col items-center gap-2"
                >
                  {isBuilding ? (
                  ...
                  <span>Simple Graph</span>
                  <span className="text-[10px] font-normal opacity-75">Fast - Direct connections only</span>
                </button>

                {/* Deep Graph */}
                <button
                  onClick={() => onBuildGraph(paper, true)}
                  disabled={isBuilding}
                  className="flex-1 px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl font-bold text-[14px] hover:from-purple-700 hover:to-indigo-700 transition-all shadow-lg shadow-purple-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex flex-col items-center gap-2"
                >
                  {isBuilding ? (
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
                  ) : (
                  ...
                  <span className="text-[10px] font-normal opacity-75">Shows inter-connections</span>
                </button>
              </div>
              
              <p className="text-xs text-slate-400 mt-4">
                Deep Graph finds connections between citations and references (takes longer)
              </p>
            </div>
          </div>
        </div>
